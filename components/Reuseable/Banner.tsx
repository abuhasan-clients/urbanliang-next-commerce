...
const Banner = ({ layer, image, defaultInfo, icon, title }: any) => {
    // className1, className2, images, title1, title2, icon, description, desClassName, mainClassName
    return (
        ...
        <div className={`relative ${styles.banner_wrapper} mb-20`}>
            <div className={`${styles.img_parent}`}>
                ...
                {defaultInfo && (
                    <div className={`${styles.banner_info} flex flex-col items-center justify-center w-full`}>
                        <i className={`${icon}`} />
                        <h1>{title}</h1>
                    </div>
                )}
